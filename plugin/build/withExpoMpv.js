...
exports.downloadMPVKit = downloadMPVKit;
...
function downloadFramework(source, frameworksDir) {
    const zipPath = path_1.default.join(frameworksDir, `${source.name}.zip`);
    (0, child_process_1.execFileSync)('curl', [
        '--location', '--fail', '--retry', '10', '--retry-all-errors',
        '--retry-delay', '2', '--continue-at', '-', '--silent', '--show-error',
        '--output', zipPath, source.url,
    ], { stdio: 'inherit' });
    (0, child_process_1.execFileSync)('unzip', ['-q', '-o', zipPath, '-d', frameworksDir]);
...
async function downloadMPVKit(frameworksDir) {
    ...
    if (fs_1.default.existsSync(lockfile) && fs_1.default.readFileSync(lockfile, 'utf-8').trim() === MPVKIT_VERSION &&
        sources.every(({ name }) => fs_1.default.existsSync(path_1.default.join(frameworksDir, `${name}.xcframework`, 'Info.plist')))) {
    ...
    for (const source of sources) {
        if (fs_1.default.existsSync(path_1.default.join(frameworksDir, `${source.name}.xcframework`, 'Info.plist')))
            continue;
